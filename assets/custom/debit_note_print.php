...
class PDF_AutoPrint extends PDF_JavaScript
{
	...
	function Header()
	{
		...
		$this->Cell(190,7,'Debit Note',0,2,C);
		...
		$this->Cell(25,5,'Debit Note No.',0,0,L);
		$temp = ':   '.$GLOBALS["dn_no"];
		...
		$this->Cell(75,5,$temp,0,1,L);
		$this->setX('102');
		$this->CellFitScale(25,5,'Purchase Invoice No',0,0,L);
		$temp = ':   '.$GLOBALS["purchase_invoice"];
		$this->Cell(75,5,$temp,0,1,L);
		$this->setX('102');
		$this->Cell(25,5,'Purchase Date',0,0,L);
		$temp = ':   '.$GLOBALS["pi_dt"];
		$this->Cell(75,5,$temp,0,1,L);
		$this->setX('102');
...
$sql = "SELECT * FROM debit_note WHERE `dn_no` = '$dn_no'";
$query = $db->query($sql);
...
$GLOBALS["dn_no"] = $dn_no;
$GLOBALS["purchase_invoice"] = $row['purchase_invoice'] ?? '';
$GLOBALS["dt"] = !empty($row['dn_date']) ? date('d-m-Y', strtotime($row['dn_date'])) : '';

if(!empty($row['dn_pi_date']))
	$GLOBALS["pi_dt"] = date('d-m-Y', strtotime($row['dn_pi_date']));


$GLOBALS['supplier'] = $row_temp['print_name'] ?? '';
...
if(($addons_array['roundoff'] ?? '') != '' && ($addons_array['roundoff'] ?? 0) != 0)
{
