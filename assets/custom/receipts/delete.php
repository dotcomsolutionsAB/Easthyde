...
$latest_id = null;
$latest_q = $db->query("SELECT id FROM receipts ORDER BY id DESC LIMIT 1");
if ($latest_q && ($latest_row = $latest_q->fetch_assoc())) {
	$latest_id = (string)($latest_row['id'] ?? '');
}

$row = null;
$query_row = $db->query("SELECT * FROM receipts WHERE id = '$safeId' LIMIT 1");
if ($query_row) {
	$row = $query_row->fetch_assoc();
}

if (!$row) {
	$output['messages'] = 'Receipt not found';
	echo json_encode($output);
	exit;
}
...
$r_no = $row['r_no'] ?? '';
$client = $row['client'] ?? '';
$siNos = receipt_si_numbers($row['sales_invoice'] ?? '');
$was_latest = ($latest_id !== null && $latest_id === $id);

$sql = "DELETE FROM receipts WHERE id = '$safeId'";
$query = $db->query($sql);

if ($query === TRUE) {
	sync_sales_documents($db, $siNos, $client);
	if ($was_latest) {
		maybe_decrement_counter($db, 'receipt', $row_counter_arr);
	}
	$output['success'] = true;
