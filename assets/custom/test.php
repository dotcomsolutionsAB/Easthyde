...
<table class="table table-bordered table-striped table-hover " >
	<thead>
	    <tr>
	      ...
	      <th scope="col" class="header">SO No</th>
	      <th scope="col" class="header">SO Date</th>
	      <th scope="col" class="header">Product Name</th>
	      <th scope="col" class="header">Quantity</th>
	      <th scope="col" class="header">Sent</th>
...
$sql = "SELECT * FROM sales_order WHERE status = '0' AND `collected` = '1'";
$query = $db->query($sql);
while($row = $query->fetch_assoc()){

    $so_items = json_decode($row['items'], true);
    $so_len = sizeof($so_items['product']);

    $so_no = $row['so_no'];

    $sql_check = "SELECT COUNT(*) AS total FROM sales_invoice WHERE so_no LIKE '%$so_no%'";
    $query_check = $db->query($sql_check);
    $row_check = $query_check->fetch_assoc();

    if($row_check['total'] > 0){

    	$sql_check = "SELECT * FROM sales_invoice WHERE so_no LIKE '%$so_no%'";
	    $query_check = $db->query($sql_check);
	    $row_check = $query_check->fetch_assoc();

    	for($ii=0;$ii<$so_len;$ii++){

    		$name = $so_items['product'][$ii];
	    	...
	    	<tr>
		      ...
		      <td><?php echo $row_check['si_no']; ?></td>
