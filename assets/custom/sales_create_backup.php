<?php
    ...
    if($id == '')
    {
        ...
        $sql = "INSERT INTO sales_invoice (`client_name`,`si_no`,`series`,`si_date`,`so_no`,`shipping`,`state`,`invoice_details`,`items`,`addons`,`hsn_table`,`total`,`tax`,`status`,`log_user`,`log_date`) VALUES ('$client','$order_no','$series', '$invoice_date','$sales_o','$address','$state','$invoice_details','$item','$addon','1','$tot_amount','$tax_json','$status','$log_user','$log_date')";
        $query = $db->query($sql);
    ...
    {
        $sql = "UPDATE sales_invoice SET `client_name` = '$client', `si_no`='$order_no',`si_date`='$invoice_date', `so_no`='$sales_o',`shipping`='$address',`state`='$state',`invoice_details`='$invoice_details',`items`='$item',`addons`='$addon', `total` = '$tot_amount', `tax` = '$tax_json',`status`='$status',`cancelled`='0',`log_user`='$log_user',`log_date`='$log_date' WHERE `id`='$id'";
        $query = $db->query($sql);
