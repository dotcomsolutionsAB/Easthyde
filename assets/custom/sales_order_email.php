...
$email_arr = trim($_REQUEST['so_em_email']);
$email_arr = str_replace(" ","",$email_arr);

$email = explode(',', $email_arr);
$len = sizeof($email);
$cc = explode(',', $_REQUEST['so_em_email_cc']);
$len_cc = sizeof($cc);
$bcc = explode(',', $_REQUEST['so_em_email_bcc']);
$len_bcc = sizeof($bcc);

$subject = $_REQUEST['so_em_subject'];
$message = $_REQUEST['so_em_message'];
...
//CC and BCC
if($_REQUEST['so_em_email_cc'] != ''){
    for($k=0;$k<$len_cc;$k++){
        $mail->addCC($cc[$k], '');     // Add a recipient
    }
}
if($_REQUEST['so_em_email_bcc'] != ''){
    for($k=0;$k<$len_bcc;$k++){
