<?php
	...
	$supplier = replace_improper((string)($_REQUEST['edit_supplier_name'] ?? ''));
	$supplier_print = replace_improper((string)($_REQUEST['edit_supplier_print_name'] ?? ''));
	$gstin = replace_improper((string)($_REQUEST['edit_supplier_gstin'] ?? ''));
	$gstin_type = replace_improper_same((string)($_REQUEST['edit_supplier_gstin_type'] ?? ''));
	$type = replace_improper((string)($_REQUEST['edit_supplier_category'] ?? ''));
	$state = replace_improper((string)($_REQUEST['edit_supplier_state'] ?? ''));
	$country = replace_improper((string)($_REQUEST['edit_supplier_country'] ?? ''));

	$credit = replace_improper((string)($_REQUEST['edit_supplier_credit'] ?? ''));
	$opening = replace_improper((string)($_REQUEST['edit_supplier_opening'] ?? ''));
	...
	$bank_details['name']=replace_improper((string)($_REQUEST['edit_bank_supplier'] ?? ''));
	...
	$address['address_1']=replace_improper((string)($_REQUEST['edit_supplier_add_1'] ?? ''));
	$address['address_2']=replace_improper((string)($_REQUEST['edit_supplier_add_2'] ?? ''));
	$address['city']=replace_improper((string)($_REQUEST['edit_supplier_city'] ?? ''));
	$address['pincode']=replace_improper((string)($_REQUEST['edit_supplier_pincode'] ?? ''));

	for($i=0;$i<$l;$i++){
        $contacts['name'][] =replace_improper((string)($array[$i]['edit_supplier_person'] ?? ''));
        $contacts['designation'][] = replace_improper((string)($array[$i]['edit_supplier_designation'] ?? ''));
        $contacts['mobile'][] = replace_improper((string)($array[$i]['edit_supplier_mobile'] ?? ''));
        $contacts['email'][] = replace_improper((string)($array[$i]['edit_supplier_email'] ?? ''));
    }
