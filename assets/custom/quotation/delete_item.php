<?php
    ...
    if (!is_array($items) || !isset($items['product']) || !is_array($items['product'])) {
        $items = ['product'=>[], 'group'=>[], 'quantity'=>[], 'unit'=>[], 'price'=>[], 'discount'=>[], 'hsn'=>[], 'tax'=>[], 'desc'=>[], 'long_desc'=>[], 'tax_amount'=>[], 'amount'=>[]];
    }
    foreach (['group','quantity','unit','price','discount','hsn','tax','desc','long_desc','tax_amount','amount'] as $key) {
