...
class PDF_AutoPrint extends PDF_JavaScript
{
	...
	function Header()
	{
	 ...
	 //    $this->Image("../media/pdf/quot_top.jpg",10,10,190,20);
		...
		$this->Cell(70,7,'Proforma Invoice',0,0,C);
...
$GLOBALS["pr_no"] = $pr_no;
$GLOBALS["dt"] = !empty($row['pr_date']) ? date('d-m-Y', strtotime($row['pr_date'])) : '';

if($row_temp && ($row_temp['print_name'] ?? '') != '')
	$GLOBALS['client'] = $row_temp['print_name'];
else
	$GLOBALS['client'] = $row['client_name'] ?? '';
...
for($i=0;$i<$l;$i++){
	...
	if($flag == 0)
	{
		...
		$temp = $items['product'][$i];
		$product = dotcom_wordwrap($temp,45);
		$co = (is_array($product) ? count($product) : 1);
...
$t_tax = is_array($items['tax'] ?? null) ? ($items['tax'][0] ?? 0) : 0;
$t2_tax = $t_tax/2;
...
if($tot_total == '0') {
	...
	$pdf->Image("../media/company-logos/company_stamp.png",170,235,22,22);
	$pdf->Cell(95,12,'','R',0,L);
	$pdf->Cell(95,12,'','',1,R);
	$pdf->Cell(95,5,'','R',1,L);
	$pdf->Cell(95,5,'','R',0,L);
	$pdf->Cell(95,5,'Authorised Signatory',0,1,R);
