...
$client_print = $row_fetch['print_name'];
...
if ($query) {
while($row = $query->fetch_assoc()){
    $result['particulars'][]    = '*Receipt*';
    $result['date'][] 			= $row['date'];
    $result['voucher'][]        = '		';
    $result['credit'][] 		= '';
    $result['debit'][] 			= $row['amount'];
    $result['details'][] 		= '_- '.$row['mode'].' ('.$row['instrument'].')_';
...
//Message Creation
$output['message'] = 'Kind Attention : *M/S '.$client_print.'*
...
$output['message'] .= '
Total Debit 	: *Rs. '.number_format((float)($debit), 2).'*
Total Credit 	: *Rs. '.number_format((float)($credit), 2).'*

Balance Due 	: *Rs. '.number_format((float)($total), 2).'*
