...
$sql = "SELECT * FROM quotation WHERE quotation_no = 'AIC/Q-0087/20-21'";
$query = $db->query($sql);
while($row = $query->fetch_assoc()){
	...
	for($i=0;$i<$len;$i++){
		...
		if($items['long_desc'][$i] != null)
			$new_items['long_desc'][$i] 	= $items['long_desc'][$i];
		else
			$new_items['long_desc'][$i] 	= "";
		if($items['group'][$i] != null)
	...
	$sql_add = "UPDATE quotation SET `items` = '$items_json',`tax` = '$tax_json',`total` = '$tot_amount',`addons` = '$addons_json' WHERE `id` = '$id'";
