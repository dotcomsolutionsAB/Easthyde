...
while($row = $query->fetch_assoc()){
    ...
    for($i=0;$i<$len;$i++){
        if($items['product'][$i] == $pr){
            ...
            $rate = $items['price'][$i];
            $amount = $items['price'][$i] * $items['quantity'][$i];
            if($items['discount'][$i] > 0){
                $amount = $amount * (100 - $items['discount'][$i]) / 100;
            }
    ...
    $result['qty'][] = $qty;
    $result['rate'][] = money_format('%!i',$amount);
}
...
while($row = $query->fetch_assoc()){
    ...
    $result['date'][] =  date('d-m-Y',strtotime($row['cn_date']));
...
while($row = $query->fetch_assoc()){
    ...
    $result['date'][] =  date('d-m-Y',strtotime($row['dn_date']));
...
while($row = $query->fetch_assoc()){
    ...
    $result['date'][] =  date('d-m-Y',strtotime($row['quotation_date']));
    $result['masters'][] = $row['client'];
...
while($row = $query->fetch_assoc()){
    ...
    $result['date'][] =  date('d-m-Y',strtotime($row['po_date']));
    $result['masters'][] = $row['supplier_name'];
...
while($row = $query->fetch_assoc()){
    ...
    $result['date'][] =  date('d-m-Y',strtotime($row['so_date']));
    $result['masters'][] = $row['client_name'];
...
while($row = $query->fetch_assoc()){
    ...
    $result['date'][] =  date('d-m-Y',strtotime($row['log_date']));
