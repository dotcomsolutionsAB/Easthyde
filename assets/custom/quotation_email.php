...
class PDF_AutoPrint extends PDF_JavaScript
{
	...
	function Header()
	{
		...
		$this->Cell(190,7,'QUOTATION',0,2,C);
...
$GLOBALS["q_no"] = $q_no;
$GLOBALS["dt"] = date('d-m-Y', strtotime($row['quotation_date']));
...
$tr_flag=0;
$tmp_y = $pdf->getY();
$terms_limit = 297 - 35 - 47 - ($terms_limit_add*4);
...
$pdf->Cell(70,5,'for AMMAR INDUSTRIAL CORPORATION',0,1,R);
...
// $sending_host = "smtp.gmail.com";
// $sending_email = "info@ammarindustrial.in";
...
$email_arr = trim($_REQUEST['q_em_email']);
$email_arr = str_replace(" ","",$email_arr);

$email = explode(',', $email_arr);
$len = sizeof($email);
$cc = explode(',', $_REQUEST['q_em_email_cc']);
$len_cc = sizeof($cc);
$bcc = explode(',', $_REQUEST['q_em_email_bcc']);
$len_bcc = sizeof($bcc);

$subject = $_REQUEST['q_em_subject'];
$message = $_REQUEST['q_em_message'];
...
//CC and BCC
if($_REQUEST['q_em_email_cc'] != ''){
    for($k=0;$k<$len_cc;$k++){
        $mail->addCC($cc[$k], '');     // Add a recipient
    }
}
if($_REQUEST['q_em_email_bcc'] != ''){
    for($k=0;$k<$len_bcc;$k++){
