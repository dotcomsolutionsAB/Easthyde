<?php
    ...
    foreach($array[purchase_invoice] as $item){
        ...
        if($item['pi_product_name'] != '' && $item['pi_qty'] != ''){
            ...
            $secondary_total = $secondary_total + $total_temp;
            ...
            if ($item['pi_dsc'] != '') {
                $total = ($item['pi_rate'] * $item['pi_qty']) - ($item['pi_rate'] * $item['pi_qty'] * $item['pi_dsc'] / 100 );
            }
    ...
    if($state == 'WEST BENGAL'){
        ...
        $addons['freight']['cgst'] = number_format($tax_value,2, '.', '');
        $addons['freight']['sgst'] = number_format($tax_value,2, '.', '');
        $tax['cgst'] += number_format($tax_value,2, '.', '');
        $tax['sgst'] += number_format($tax_value,2, '.', '');
        ...
        $addons['pf']['cgst'] = number_format($tax_value,2, '.', '');
        $addons['pf']['sgst'] = number_format($tax_value,2, '.', '');
        $tax['cgst'] += number_format($tax_value,2, '.', '');
        $tax['sgst'] += number_format($tax_value,2, '.', '');
        ...
        $addons['freight']['igst'] = number_format($tax_value,2, '.', '');
        $tax['igst'] += number_format($tax_value,2, '.', '');
        ...
        $addons['pf']['igst'] = number_format($tax_value,2, '.', '');
        $tax['igst'] += number_format($tax_value,2, '.', '');
    ...
    if($pi_id == '')
    {
        ...
        $sql = "INSERT INTO purchase_invoice (`supplier_name`,`pi_no`,`pi_date`,`po_no`,`shipping`,`items`,`addons`,`total`,`tax`,`status`,`log_user`,`log_date`,`series`) VALUES ('$supplier','$order_no', '$order_date','$purchase_o','$address','$item','$addon','$tot_amount','$tax_json','$status','$log_user','$log_date','$series')";
        $query = $db->query($sql);
        ...
        $sql = "UPDATE purchase_invoice SET `supplier_name` = '$supplier', `pi_no`='$order_no',`pi_date`='$order_date', `po_no`='$purchase_o',`shipping`='$address',`items`='$item',`addons`='$addon', `total` = '$tot_amount', `tax` = '$tax_json',`log_user`='$log_user',`log_date`='$log_date',`series`='$series' WHERE `id`='$pi_id'";
        $query = $db->query($sql);
