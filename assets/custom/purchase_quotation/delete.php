...
$row_tmp = ($query_tmp && ($tmp = $query_tmp->fetch_assoc())) ? $tmp : null;

if ($row_tmp) {
    $series = $row_tmp['series'] ?? '';

    $po_numbers = json_decode($row_tmp['pq_no'] ?? '', true);
    if (!is_array($po_numbers)) { $po_numbers = []; }
    ...
    for($i=0;$i<$len;$i++){
        ...
        if ($query_temp) {
            while($row_temp = $query_temp->fetch_assoc()){
                ...
                $l = (isset($items['product']) && is_array($items['product'])) ? sizeof($items['product']) : 0;
                ...
                if ($query1) {
                    while($row1 = $query1->fetch_assoc())
                    {
                        ...
                        if (!is_array($items1)) { $items1 = []; }
                        $len = (isset($items1['product']) && is_array($items1['product'])) ? sizeof($items1['product']) : 0;
                        for($j=0;$j<$len;$j++)
                        {
                            $pr = $items1['product'][$j];
