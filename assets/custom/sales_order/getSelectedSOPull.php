...
for ($i = 0; $i < $len; $i++) {
    $member = $memberId[$i];
    $sql = "SELECT * FROM sales_order WHERE so_no = '$member'";
    $query = $db->query($sql);
    ...
    $client_so_no = $result['client_so_no'] ?? '';
    $so_date = $result['so_date'] ?? '';
    $q_arr = json_decode($result['q_no'] ?? '', true);
    $q_no = (is_array($q_arr) && isset($q_arr[0])) ? $q_arr[0] : '';
...
$data = array('items' => json_encode($final_items), 'addons' => json_encode($final_addons), 'client_so_no' => $client_so_no, 'so_date' => $so_date, 'q_no' => $q_no, 'mobile' => $mobile);
