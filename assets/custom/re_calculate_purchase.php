...
while($row = $query->fetch_assoc()){
	...
	for($i=0;$i<$len;$i++){
		...
		if($state == "WEST BENGAL"){
			...
			if($addons['freight']['value'] != '0' && $addons['freight']['value']['value'] != '0.00' && $addons['freight']['value'] != '')
			...
			$new_addons['pf']['value'] = $addons['pf']['value']['value'];
	...
	$add_fraction 		= $addons['roundoff'];
	$tot_amount 		+= $add_fraction;

    $new_addons['roundoff'] = $addons['roundoff'];
	...
	$sql_add = "UPDATE purchase_invoice SET `items` = '$items_json',`tax` = '$tax_json',`total` = '$tot_amount',`addons` = '$addons_json' WHERE `id` = '$id'";
