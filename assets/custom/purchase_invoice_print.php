...
$GLOBALS["pi_no"] = $pi_no;
$GLOBALS["dt"] = !empty($row['pi_date']) ? date('d-m-Y', strtotime($row['pi_date'])) : '';
...
if(($addons_array['tcs'] ?? '')!='' && (float)($addons_array['tcs'] ?? 0) > 0){
	...
	$pdf->SetFont('Arial','',9);
	$pdf->Cell(23,5,number_format((float)$addons_array['tcs'], 2),0,1,R);
	
}

$tax_temp = json_decode($row['tax'] ?? '', true);
if (!is_array($tax_temp)) { $tax_temp = []; }

$sgst = $tax_temp['cgst'] ?? 0;
$cgst = $tax_temp['sgst'] ?? 0;
$igst = $tax_temp['igst'] ?? 0;

// $sgst = $GLOBALS["gross_total"] * 9 / 100;
// $cgst = $GLOBALS["gross_total"] * 9 / 100;
// $igst = $GLOBALS["gross_total"] * 18 / 100;
