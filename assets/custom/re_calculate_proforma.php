...
$sql = "SELECT * FROM proforma WHERE so_no = 'AIC/PI-033/20-21'";
$query = $db->query($sql);
while($row = $query->fetch_assoc()){
	...
	for($i=0;$i<$len;$i++){
		...
		if($state == "WEST BENGAL"){
			...
			$new_addons['freight']['value'] = $addons['freight'];

			if($addons['freight'] != '0' && $addons['freight'] != '0.00' && $addons['freight'] != '')
				$tax_value = $addons['freight'] * 9 / 100;
			else
				$tax_value = 0;
			$new_addons['freight']['cgst'] = round($tax_value,2);
			$new_addons['freight']['sgst'] = round($tax_value,2);

			$new_addons['pf']['value'] = $addons['pf'];

			if($addons['pf'] != '0' && $addons['pf'] != '0.00' && $addons['pf'] != '')
				$tax_value = $addons['pf'] * 9 / 100;
			else
				$tax_value = 0;
			...
			if($addons['freight'] != '0' && $addons['freight'] != '0.00' && $addons['freight'] != '')
				$tax_value = $addons['freight'] * 18 / 100;
			else
				$tax_value = 0;
	...
	$sql_add = "UPDATE proforma SET `items` = '$items_json',`tax` = '$tax_json',`total` = '$tot_amount',`addons` = '$addons_json' WHERE `id` = '$id'";
